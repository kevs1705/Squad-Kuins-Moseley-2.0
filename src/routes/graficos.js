...
router.get("/api/graficos/analytics", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    // 1. Filtrar Pasantes en Scope
    let userWhere = "u.rol = 0";
    const userParams = [];

    if (modo === "carrera" && carreraId) {
      userWhere += " AND (u.id_carrera = ? OR u.carrera = ?)";
      userParams.push(carreraId, carreraId);
    } else if (modo === "estudiante" && estudianteId) {
    ...
    const [pasantes] = await db.query(
      `SELECT u.id_usuario, 
              CONCAT(u.nombre, ' ', COALESCE(u.apellido_paterno,'')) AS nombre_completo,
              u.CI, u.universidad, COALESCE(u.id_carrera, u.carrera) AS id_carrera,
    ...
    const rankingEstudiantes = pasantes
      .map((p) => {
...
router.get("/api/graficos/usuarios", async (_req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT id_usuario, nombre FROM usuarios WHERE rol = 0 ORDER BY nombre`
    );
